...
class CreateGeradorCommand extends Command
{

    protected $name = 'gerador:make-model';
    protected $description = 'Gere um model';
    protected $command;
    protected $files;
    protected $pk_field;
    ...
    public function handle()
    {
        ...
        if(empty($tabela)){
            echo 'Utilize a opção --tabela=? Exemplo:php artisan gerador:make example --tabela=cli';
            die();
        }
        ...
        try {
            ...
            $stub_model  = $this->files->get($this->getStub('model.stub'));
            $stub_var = array(
                "%MODELNAME%",
                '%TABLENAME%',
                "%TABLEPK%",
            ...
            $file_var   = array(
                ...
                $tabela,
                $this->pk_field,
                config('gerador.namespace')."\\".ucfirst($modulo)."\\Entities",
                $fillable
            ...
            $path = $this->getDestinationFilePath(ucfirst($modulo),ucfirst($tabela));
    ...
    protected function getDestinationFilePath($module,$model)
    {
        $path = $this->laravel['modules']->getModulePath($module);

        $modelPath = GenerateConfigReader::read('model');

        return $path . $modelPath->getPath() . '/' . $model . '.php';
    }
