...
class CreateViewCreateCommand extends Command
{

    protected $name = 'gerador:make-view-form';
    ...
    public function handle()
    {
        ...
        try {
           ...
           // $fillable = substr($fillable, 0, -1);
            ...
            $contents_edit = $this->replace($stub_model_edit,$stub_var,$file_var);

            $path = $this->getDestinationFilePath(ucfirst($modulo),'show');
            ...
            $path = $this->getDestinationFilePath(ucfirst($modulo),'create');
            if (!$this->laravel['files']->isDirectory($dir = dirname($path))) {
                $this->laravel['files']->makeDirectory($dir, 0777, true);
            }
            ...
            ##GERA O EDIT
            $path_edit = $this->getDestinationFilePath(ucfirst($modulo),'edit');
            if (!$this->laravel['files']->isDirectory($dir = dirname($path))) {
                $this->laravel['files']->makeDirectory($dir, 0777, true);
            }
            ...
            try {
                $overwriteFile = config('gerador.force_create_file');
                (new FileGenerator($path_edit, $contents_edit))->withFileOverwrite($overwriteFile)->generate();

                $this->info("Created CRUD LARAVEL ***: {$path_edit}");
            } catch (FileAlreadyExistException $e) {
                $this->error("File : {$path_edit} already exists.");
            }
